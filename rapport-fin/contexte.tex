...
\subsection{Anatomie d'un neurone} % (fold)
\label{sub:Anatomie d'un neurone}

\begin{figure}[H]
   \begin{center}
      \psfig{width=12cm,figure=pic/neurone.eps}
   \end{center}
...
Un neurone est une cellule dont le rôle est de recevoir et transmettre de l'information.
...
% subsection Anatomie d'un neurone (end)
...
contractions et de décontractions des muscles dorsaux et ventraux générant un
...
neurone. C'est pourquoi nous ne les distinguerons pas dans la suite de ce document.
...
Les interneurones AVB et PVC sont reliés aux neurones de type B. Réciproquement, AVA et AVD
...
le corps du ver (force, élasticité) et les forces en jeu dans l'environnement.
...
\begin{figure}[ht]
   ...
   (cellules losanges).  Les synapses excitatrices sont légendées à l'aide
...
python ce qui permet de mettre à jour l'état des récepteurs de torsion du ver.
...
faibles sont automatiquement considérées comme insignifiantes, de même que pour
 la taille des ressorts constituant les muscles. Nous avons donc modifié
