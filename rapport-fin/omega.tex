...
Comme vu précédemment, il n'y a pas de données biologiques associées à la
...
ce type d'information. (1) En premier lieu l'algorithme sera testé sur un
apprentissage non supervisé de la variation de la taille des muscles du ver.
Il devra arriver à prédire avec une marge d'erreur acceptable la taille lors
du mouvement du ver. Puis, une fois ceci fait, (2) nous passerons l'algorithme
en mode supervisé pour le contraindre à générer les séquences neuronales connues
lors de mouvement connu. Le but étant que la qualité de sa prédiction de
taille ne change pas, mais que dans le même temps son état interne sois
contraint à correspondre à l'activité neuronale du ver simulé. (3) Enfin on
générera les séquences neuronales lors de mouvement non connu à l'aide du filtre; et on
validera celle-ci en fournissant au simulateur la séquence neuronale générée et
en visualisant si le modèle se comporte correctement.
...
du filtre pour le rendre supervisé.
...
demanderas l'extraction du mouvement du ver depuis des vidéos de déplacement à
